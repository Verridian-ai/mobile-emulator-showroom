...
class EnhancedProtocolIntegrationBridge {
  ...
  async detectClientCapabilities(clientInfo) {
    ...
    if (clientInfo.supportsIDE) {
      detectedFeatures.push('ide_integration');
    }
    if (clientInfo.supportsScreenshots) {
      detectedFeatures.push('screenshot_capture');
    }
    if (clientInfo.supportsCollaboration) {
      detectedFeatures.push('real_time_collaboration');
    }
    if (clientInfo.supportsElementSelection) {
      detectedFeatures.push('element_selection');
    }
