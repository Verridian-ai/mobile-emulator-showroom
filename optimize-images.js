...
async function optimizeImage(config) {
    ...
    try {
        ...
        console.log(`PNG reduction: ${originalSize}KB → ${pngSize}KB (${Math.round((1 - pngSize/originalSize) * 100)}% reduction)`);
        ...
        console.log(`WebP reduction: ${originalSize}KB → ${webpSize}KB (${Math.round((1 - webpSize/originalSize) * 100)}% reduction)`);
        ...
        console.log(`✓ PNG target (${config.targetSize}KB): ${pngTargetMet ? 'MET' : 'EXCEEDED'} (${pngSize}KB)`);
        console.log(`✓ WebP target (${config.targetSize}KB): ${webpTargetMet ? 'MET' : 'EXCEEDED'} (${webpSize}KB)`);
...
async function main() {
    ...
    results.forEach(result => {
        if (result.success) {
            ...
            console.log(`  PNG Optimized: ${result.pngOptimized}KB (-${Math.round((1 - result.pngOptimized/result.original) * 100)}%)`);
            console.log(`  WebP Optimized: ${result.webpOptimized}KB (-${Math.round((1 - result.webpOptimized/result.original) * 100)}%)`);
        } else {
    ...
    console.log(`  Total PNG Optimized: ${formatBytes(totalPngOptimized * 1024)} (-${Math.round((1 - totalPngOptimized/totalOriginal) * 100)}%)`);
    console.log(`  Total WebP Optimized: ${formatBytes(totalWebpOptimized * 1024)} (-${Math.round((1 - totalWebpOptimized/totalOriginal) * 100)}%)`);
